				...
				for(Route route : routes){
					...
					if(Routes.getInstance().isRouteClaimed(route) && route.getOwner() != null){
						if(route.getOwner().getName().equals(greenPlayerName)){
							if(greenCheck != null) greenCheck.setVisible(true);
						}
						else{
							if(greenCheck != null) greenCheck.setVisible(false);
						}
						
						if(route.getOwner().getName().equals(orangePlayerName)){
							if(orangeCheck != null) orangeCheck.setVisible(true);
						}
						else{
							if(orangeCheck != null) orangeCheck.setVisible(false);
						}
					}
				}
			}
